...
namespace Migrator
{
    ...
    class Program
    {
        ...
        static string mongoConnectionString = "mongodb+srv://m220student:m220password@mflix.epver.mongodb.net/myFirstDatabase?retryWrites=true&w=majority";
        ...
        static async Task Main(string[] args)
        {
            ...
            var list = new List<ReplaceOneModel<Movie>>(datePipelineResults.Count);
            if (datePipelineResults.Count > 0)
            {
                ...
                foreach (var movie in datePipelineResults)
                {
                    list.Add(new ReplaceOneModel<Movie>(Builders<Movie>.Filter.Where(x => x.Id == movie.Id), movie));
                }
                bulkWriteDatesResult = await _moviesCollection.BulkWriteAsync(
                    list,
            ...
            var list2 = new List<ReplaceOneModel<Movie>>(ratingPipelineResults.Count);
            if (ratingPipelineResults.Count > 0)
            {
                ...
                foreach (var movie in ratingPipelineResults)
                {
                    list2.Add(new ReplaceOneModel<Movie>(Builders<Movie>.Filter.Where(x => x.Id == movie.Id), movie));
                }
                bulkWriteRatingsResult = await _moviesCollection.BulkWriteAsync(list2, new BulkWriteOptions() { IsOrdered = false });
