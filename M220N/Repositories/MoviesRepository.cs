    ...
    public class MoviesRepository
    {
                 ...
                 * catch (Exception ex)
                    {
                      if (ex.GetType() == typeof(FormatException)
                        && ex.Message.Contains("is not a valid 24 digit hex string."))
                      {
                          return null;
                      }
                    ...
                    // NOTE: You can simplify this catch block by catching the more specific
                    // System.FormatException instead of the general catch statement,
                    // and then you just need to compare the message contents:
            ...
            )
        {
            ...
            var movies = await _moviesCollection.Find<Movie>(filter)
                ...
                .Project<MovieByCountryProjection>(projectionFilter).ToListAsync(cancellationToken);
            return movies;
            ...
            int page = 0, params string[] genres)
        {
            ...
            var toReturn = await _moviesCollection.Find(Builders<Movie>.Filter.In("genres", genres))
                .Limit(limit)
                .Skip(page * limit)
